...
def main():
    ...
    with Pool(args.threads, initializer=init):
...
def setup():
    ...
    parser.add_argument('--sort_all', action='store_true',
                        ...
                        'Otehrwise consider question words first.')
    parser.add_argument('sample_size', tpye=int, default=0,
    ...
    for article in data:
        for paragraph in article['paragraphs']:
            ...
            for qa in paragraph['qas']:
                id_, question, answers = qa['id'], qa['question'], qa['answer']
                if mode == 'train':
...
def annotate(row, wv_cased):
    ...
    return (id_, context_tokens, context_features, context_tags, context_ents,
            question_tokens, context, context_tokens_lower) + row[3:]
...
def build_vocab(questions, contexts, wv_vocab, sort_all=False):
    ...
    if sort_all:
        ...
        vocab = sorted([t for t in counter if t in wv_vocab], key=counter.get, reversed=True)
    else:
        counter_q = collections.Counter(w for doc in questions for w in doc)
        counter_c = collections.Counter(w for doc in contexts for w in doc)
        counter = counter_c + counter_q
        vocab = sorted([t for t in counter_q if t in wv_vocab], key=counter_q.get, reversed=True)
