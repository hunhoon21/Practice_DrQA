...
class DocReaderModel(object):
    ...
    def __init__(self, opt, embedding=None, state_dict=None):
        ...
        self.updates = state_dict['update'] if state_dict else 0
    ...
    def update(self, ex):
        ...
        target_e = ex[8].to(self.divice)

        # Run Forward, 근데 왜 forward가 실행이 될까?
        ...
        # Clear grads, Run Backward
        self.optimizer.zero_grad()
        loss.backward()

        # Clip gradients, 무슨 역할을 할까?
        torch.nn.utils.clip_grad_norm_(self.network.parameters(),
                                       self.opt['gradient_clipping'])
    ...
    def predict(self, ex):
        ...
        if self.opt['cuda']:
            ...
            inputs = [torch.tensor(e.cuda(async=True)) for e in ex[:7]]
        else:
            #inputs = [Variable(e) for e in ex[:7]]
            inputs = [torch.tensor(e) for e in ex[:7]]
        ...
        for i in range(score_s.size(0)):
            ...
            scores = scores.numpy()
            s_idx, e_dix = np.unravel_index(np.argmax(scores), scores.shape)
            s_offset, e_offset = spans[i][s_idx][0], spans[i][e_dix][1]
